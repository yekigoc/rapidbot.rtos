...
static void prvHandleStandardVendorRequest( xUSB_REQUEST *pxRequest );
...
unsigned portLONG ulRxBytes;
	...
	if( pxMessage->ulCSR0 & AT91C_UDP_RXSETUP )
	{
		...
		{		 		
			...
			{
			  char b[128];
			  //			  sprintf (b, "%i", pxControlRx.ulTotalDataLength);
			  //LCDPutStr(b, 40, 5, SMALL, BLACK, WHITE);
			  //sprintf (b, "%i", xRequest.usLength);
			  //LCDPutStr(b, 50, 5, SMALL, BLACK, WHITE);
		...
		&& ( pxControlRx.ulNextCharIndex >= pxControlRx.ulTotalDataLength ) )
	{
		...
		//		//LCDPutStr("a", 20, 20, SMALL, BLACK, WHITE);
		switch( ucRequest )
		{
			case usbSTANDARD_DEVICE_REQUEST:	
				/* Standard Device request */
			  //LCDPutStr("0", 25, 20, SMALL, BLACK, WHITE);
				prvHandleStandardDeviceRequest( &xRequest );
				break;

			case usbSTANDARD_INTERFACE_REQUEST:	
				/* Standard Interface request */
			  //LCDPutStr("4", 20, 28, SMALL, BLACK, WHITE);
				prvHandleStandardInterfaceRequest( &xRequest );
				break;

			case usbSTANDARD_END_POINT_REQUEST:	
				/* Standard Endpoint request */
			  //LCDPutStr("+", 30, 20, SMALL, BLACK, WHITE);
				prvHandleStandardEndPointRequest( &xRequest );
				break;

			case usbSTANDARD_VENDOR_REQUEST:	
				/* Standard Endpoint request */
			  //LCDPutStr("3", 20, 36, SMALL, BLACK, WHITE);
				prvHandleStandardVendorRequest( &xRequest );
				break;

			case usbCLASS_INTERFACE_REQUEST:	
				/* Class Interface request */
			  //LCDPutStr("=", 35, 20, SMALL, BLACK, WHITE);
				prvHandleClassInterfaceRequest( &xRequest );
				break;

			default:	/* This is not something we
					   want to respond to. */
			  //LCDPutStr("x", 20, 20, SMALL, BLACK, WHITE);
				prvSendStall();	
		}
	}
}
...
static void prvHandleStandardVendorRequest( xUSB_REQUEST *pxRequest )
{
  ...
  //LCDPutStr(rep, 10, 10, SMALL, BLACK, WHITE);
  memset(rep,"a",2);
  switch( pxRequest->ucRequest )
    {
    case usbF0_REQUEST:
      /* Just send two byte dummy status. */
      prvSendControlData( ( unsigned portCHAR * ) rep, sizeof( rep ), sizeof( rep ), pdFALSE );
      break;
...
unsigned portSHORT usStatus = 0;
	...
	switch( pxRequest->ucRequest )
	{
		...
		case usbSET_FEATURE_REQUEST:
		  //LCDPutStr("4", 90, 50, SMALL, BLACK, WHITE);
			prvSendZLP();
			break;

		case usbSET_ADDRESS_REQUEST:			
		  //LCDPutStr("6", 90, 60, SMALL, BLACK, WHITE);
		...
		case usbSET_CONFIGURATION_REQUEST:
		  //LCDPutStr("7", 90, 70, SMALL, BLACK, WHITE);
		...
		default:
		  //LCDPutStr("8", 90, 80, SMALL, BLACK, WHITE);
			/* Any unsupported request results in a STALL response. */
			prvSendStall();
			break;
	}
	...
	switch( pxRequest->ucRequest )
	{
		case usbGET_STATUS_REQUEST:
		  ...
		  switch (pxRequest->usValue)
		    {
		    case 0x1:
		      prvSendControlData( ( unsigned portCHAR * ) &trspistat.spiisrcalls, sizeof( trspistat.spiisrcalls ), sizeof( trspistat.spiisrcalls ), pdFALSE );
		      trspistat.usbinited = 1;
		      break;
		    case 0x2:
		      prvSendControlData( ( unsigned portCHAR * ) &trspistat.trinited, sizeof( trspistat.trinited ), sizeof( trspistat.trinited ), pdFALSE );
		      break;
		    case 0x3:
		...
		case usbGET_DESCRIPTOR_REQUEST:
		  //LCDPutStr("i", 100, 10, SMALL, BLACK, WHITE);
		...
		case usbGET_INTERFACE_REQUEST:
		  //LCDPutStr("j", 100, 30, SMALL, BLACK, WHITE);
		  break;
		case usbSET_FEATURE_REQUEST:
		  //LCDPutStr("k", 100, 40, SMALL, BLACK, WHITE);
		  break;
		case usbSET_INTERFACE_REQUEST:	
		  //LCDPutStr("l", 100, 50, SMALL, BLACK, WHITE);
		  //			prvSendZLP();
		  break;
