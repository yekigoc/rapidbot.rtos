...
static int get_hwstat(unsigned char *status)
{
  ...
  while (1)
      ...
      if (usbdataready == 1)
	{
	  ...
	  memset (buf, 0, 256);
	  sprintf (buf, "logs/log%.4i.gsh",j);
	  FILE * gp = fopen(buf, "w");
	  fprintf (gp, "set terminal png\n");
	  fprintf (gp, "#set encoding iso_8859_5\n");
	  fprintf (gp, "set output \"sygnal_%.4i.png\"\n", j);
	  fprintf (gp, "set grid\n");
	  fprintf (gp, "plot \"log%.4i.dat\" with lines t \"сигнал ч.1\", \"log%.4i.dat\" with lines t \"сигнал ч.2\", \"log%.4i.dat\" with lines t \"сигнал ч.3\"\n", j, j+1, j+2); 

	  fclose(gp);
	  memset (buf, 0, 256);
	  sprintf (buf, "logs/log%.4i.dat", j);
	  FILE * f = fopen (buf, "w");
	  j++;
	      
	  for (int g = 0; g<255;g++)
	    {
	      fprintf (f,"%i %1.2f\n", i, ((double)adcbuf[g])/3300.0);
	      fflush(f);
	      i++;
	    }
	  fclose(f);
